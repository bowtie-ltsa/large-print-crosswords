...
class Uni extends Puz {
    constructor(puzDate) {
        super(puzDate)
    }

    async fetch() {
        console.log(`Fetching ${this.puzType} puzzle for date ${this.puzDate}...`)
        try {
            const url = this.buildUrl(this.puzDate)
            console.log(`Fetching puzzle from URL: ${url}`)
            this.raw = await this.fetchFromUrl(url)
        } catch (error) {
            console.error('Error fetching puzzle:', error)
            throw error
        }
        console.log(`Fetched ${this.puzType} puzzle for date ${this.puzDate}.`)
    }

    buildUrl(dateString) {
        console.log(`Building url for date: ${dateString}`)
        const pathpart = 'U2FsdGVkX18YuMv20%2B8cekf85%2Friz1H%2FzlWW4bn0cizt8yclLsp7UYv34S77X0aX%0Axa513fPTc5RoN2wa0h4ED9QWuBURjkqWgHEZey0WFL8%3D'
        return `https://embed.universaluclick.com/c/uucom/l/${pathpart}/g/fcx/d/${dateString}/data.json`;
    }
    ...
    async fetchFromUrl(url) {
        try {
            ...
            throw new Error(`Error fetching URL '${url}'`, { cause: error })
        }
    }

    mock() {
        console.log(`Mocking fetch of "${this.puzType}" puzzle for date ${this.puzDate}...`)
        const url = this.buildUrl(this.puzDate)
